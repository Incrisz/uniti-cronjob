...
```bash
docker build -t uniti-deployer .
```

Run with your environment (including AWS credentials and DB connection):
...
If you prefer to reuse an existing AWS profile, mount it instead of passing keys directly:
...
The image includes the AWS CLI so the app can invoke your Lambda. Set `LAMBDA_FUNCTION_NAME`, `LAMBDA_PAYLOAD`, and `DATABASE_URL` in `.env` (or as individual `-e` flags) before running.
